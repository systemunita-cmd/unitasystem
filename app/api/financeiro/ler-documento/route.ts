...
export async function POST(req: NextRequest) {
  ...
  if (!token || !supabaseUrl || !supabaseAnon) return NextResponse.json({ error: "Não autorizado." }, { status: 401 });
  ...
  const [{ data: usuario, error: authError }, { data: permitido }] = await Promise.all([
    cliente.auth.getUser(token),
    cliente.rpc("usuario_pode_acessar_financeiro"),
  ]);
  if (authError || !usuario.user || !permitido) return NextResponse.json({ error: "Acesso financeiro não autorizado." }, { status: 403 });
