...
export default function CobrancaPage() {
  ...
  return (
    <div style={{ padding: isMobile ? 14 : 24, background: "#f8fafc", minHeight: "100vh", display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {/* 🧭 ADIÇÃO: navegação pras páginas da Cobrança */}
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: isMobile ? 10 : 14 }}>
        {([
          { rota: "/crm/cobranca/dashboard",   icone: "📊", titulo: "Dashboard",        desc: "Contagem, dias de pagamento e próximos vencimentos", cor: "#2563eb", bg: "#eff6ff", bd: "#bfdbfe" },
          { rota: "/crm/cobranca/negociacoes", icone: "🤝", titulo: "Negociações",      desc: "Faturas em promessa, acordo ou negociação",          cor: "#7c3aed", bg: "#f5f3ff", bd: "#ddd6fe" },
          { rota: "/crm/cobranca/atualizacao", icone: "📤", titulo: "Atualizar planilha", desc: "Suba o status das faturas e puxe o custcode",        cor: "#16a34a", bg: "#f0fdf4", bd: "#bbf7d0" },
        ]).map(n => (
          <button key={n.rota} onClick={() => router.push(n.rota)} style={{ ...cardStyle, padding: 16, display: "flex", alignItems: "center", gap: 12, cursor: "pointer", textAlign: "left", borderLeft: `4px solid ${n.cor}` }}>
            <div style={{ width: 42, height: 42, borderRadius: 11, background: n.bg, border: `1px solid ${n.bd}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>{n.icone}</div>
            <div style={{ minWidth: 0 }}>
              <div style={{ color: "#1f2937", fontSize: 14, fontWeight: 700 }}>{n.titulo} <span style={{ color: n.cor }}>→</span></div>
              <div style={{ color: "#9ca3af", fontSize: 11.5, lineHeight: 1.4 }}>{n.desc}</div>
            </div>
          </button>
        ))}
      </div>
                      ...
                      <button key={c.proposta.id} onClick={() => { setClienteSel(c.proposta.id); setSelecionadasFat(new Set()); }}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
                            ...
                            {c.proposta.dados_customizados?.custcode && <span style={{ fontFamily: "monospace", fontSize: 10.5, color: "#2563eb", background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 5, padding: "1px 5px", fontWeight: 700, flexShrink: 0 }}>{c.proposta.dados_customizados.custcode}</span>}
                          </div>
                            ...
                            <tr key={k}
                              ...
                              <td style={{ padding: "12px", maxWidth: 220, overflow: "hidden" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                  <span style={{ color: "#1f2937", fontSize: 13, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{f.proposta.nome || "—"}</span>
                                  {f.proposta.dados_customizados?.custcode && <span style={{ fontFamily: "monospace", fontSize: 10.5, color: "#2563eb", background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 5, padding: "1px 5px", fontWeight: 700, flexShrink: 0 }}>{f.proposta.dados_customizados.custcode}</span>}
                                </div>
