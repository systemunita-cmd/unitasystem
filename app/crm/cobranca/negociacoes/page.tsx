...
export default function CobrancaPage() {
  ...
  const [exportEscopo, setExportEscopo] = useState<"filtros" | "geral" | "mes" | "periodo">("filtros");
  const [exportMesInst, setExportMesInst] = useState("");
  const [exportDataInicio, setExportDataInicio] = useState("");
  const [exportDataFim, setExportDataFim] = useState("");
  const [exportAbas, setExportAbas] = useState({
    clientes: true,
    faturas: true,
    tabulacao: true,
    filtros: true,
  ...
  const clientesParaExportar = (): any[] => {
    if (exportEscopo === "filtros") return clientesTabela;
    let arr = clientes;
    if (exportEscopo === "mes" && exportMesInst) {
      arr = arr.filter(c => String(c.proposta.data_instalacao || "").startsWith(exportMesInst));
    }
    if (exportEscopo === "periodo") {
      arr = arr.filter(c => {
        const data = String(c.proposta.data_instalacao || "").slice(0, 10);
        if (exportDataInicio && (!data || data < exportDataInicio)) return false;
        if (exportDataFim && (!data || data > exportDataFim)) return false;
        return true;
      });
    }
    return arr;
  ...
  const exportarExcel = () => {
    if (exportEscopo === "mes" && !exportMesInst) {
      setFeedback({ tipo: "aviso", titulo: "Escolha um mês", mensagem: "Selecione o mês de instalação antes de gerar o Excel." });
      return;
    }
    if (exportEscopo === "periodo" && !exportDataInicio && !exportDataFim) {
      setFeedback({ tipo: "aviso", titulo: "Escolha um período", mensagem: "Informe pelo menos uma data inicial ou final para exportar por período." });
      return;
    }
    const base = clientesParaExportar();
    if (base.length === 0) {
      setFeedback({ tipo: "aviso", titulo: "Nada para exportar", mensagem: "Nenhum cliente encontrado com as opções escolhidas." });
      return;
    }
    if (!exportAbas.clientes && !exportAbas.faturas && !exportAbas.tabulacao && !exportAbas.filtros) {
      setFeedback({ tipo: "aviso", titulo: "Escolha pelo menos uma aba", mensagem: "Marque Clientes, Faturas, Tabulação ou Filtros usados antes de exportar." });
      return;
    }
    if (!exportCampos.cliente && !exportCampos.cobranca && !exportCampos.fpd) {
      setFeedback({ tipo: "aviso", titulo: "Escolha pelo menos um grupo de dados", mensagem: "Marque Dados do cliente, Resumo de cobrança ou Dados FPD." });
      return;
    }
    setExportandoExcel(true);
    try {
      const montarClienteRow = (c: any) => {
        const row: Record<string, any> = {};
        if (exportCampos.cliente) {
          Object.assign(row, {
      ...
      const clientesRows = base.map(montarClienteRow);
      const faturasRows = base.flatMap(c => {
      ...
      const filtrosRows = [{
        ...
        "Modo de exportacao": exportEscopo,
        "Status": filtroStatus,
        ...
        "Inicio escolhido no modal": exportDataInicio || "",
        "Fim escolhido no modal": exportDataFim || "",
        "Filtro nome": colNome || "",
        "Filtro OS": colOs || "",
      ...
      if (exportAbas.tabulacao) XLSX.utils.book_append_sheet(wb, wsTab, "Tabulacao mensal");
      if (exportAbas.filtros) XLSX.utils.book_append_sheet(wb, wsFiltros, "Filtros usados");

      const sufixo = exportEscopo === "periodo" ? `${exportDataInicio || "inicio"}_${exportDataFim || "fim"}`
        : exportEscopo === "mes" ? (exportMesInst || "mes")
        : exportEscopo;
    ...
    <div style={{ padding: isMobile ? 14 : 24, background: "#f8fafc", minHeight: "100vh", display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ ...cardStyle, padding: isMobile ? 16 : 20, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => setShowExportModal(true)} disabled={clientes.length === 0}
            style={{
              background: clientes.length === 0 ? "#f3f4f6" : "#f0fdf4",
              color: clientes.length === 0 ? "#9ca3af" : "#16a34a",
              border: `1px solid ${clientes.length === 0 ? "#e5e7eb" : "#bbf7d0"}`,
              borderRadius: 10,
              padding: "9px 14px",
              fontSize: 12.5,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ ...cardStyle, padding: isMobile ? 10 : 12, display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                ...
                {(dataInstInicio || dataInstFim) && (
                  <button onClick={() => { setDataInstInicio(""); setDataInstFim(""); }} style={{ background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", borderRadius: 20, padding: "5px 12px", fontSize: 12, cursor: "pointer", fontWeight: 700 }}>✕ Limpar período</button>
                )}
                <button onClick={() => setShowExportModal(true)} disabled={clientes.length === 0}
                  style={{
                    marginLeft: "auto",
                    background: clientes.length === 0 ? "#f3f4f6" : "linear-gradient(135deg, #16a34a 0%, #22c55e 100%)",
                    color: clientes.length === 0 ? "#9ca3af" : "#ffffff",
                    border: "none",
                    borderRadius: 10,
                    padding: "8px 14px",
                    fontSize: 12.5,
                    cursor: clientes.length === 0 ? "not-allowed" : "pointer",
                    fontWeight: 800,
                    boxShadow: clientes.length === 0 ? "none" : "0 4px 12px rgba(22,163,74,0.28)",
                    whiteSpace: "nowrap",
                  }}>
                  Exportar Excel
                </button>
      ...
      {showExportModal && (
        <div
          onClick={() => !exportandoExcel && setShowExportModal(false)}
          style={{
            position: "fixed",
            inset: 0,
        ...
        >
          ...
          >
            <div style={{ padding: "22px 24px 16px", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", gap: 16 }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 22, fontWeight: 900, color: "#0f172a" }}>Exportar Excel</h3>
                <p style={{ margin: "6px 0 0", color: "#64748b", fontSize: 13, fontWeight: 600 }}>
                  Marque exatamente o que deseja exportar: dados, datas, abas e campos.
                </p>
              </div>
              <button
                type="button"
            ...
            <div style={{ padding: 24, display: "grid", gap: 22 }}>
              <div>
                <label style={{ display: "block", marginBottom: 10, color: "#334155", fontWeight: 900, fontSize: 13 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
                  {([
                    ["filtros", "Tela atual"],
                    ["geral", "Geral"],
                    ["mes", "Mês de instalação"],
                    ["periodo", "Período específico"],
                  ] as [typeof exportEscopo, string][]).map(([valor, label]) => (
                    <button
                      key={valor}
                      type="button"
                      onClick={() => setExportEscopo(valor)}
                      style={{
                        padding: "12px 14px",
                        borderRadius: 14,
                        border: exportEscopo === valor ? "2px solid #2563eb" : "1px solid #dbeafe",
                        background: exportEscopo === valor ? "#eff6ff" : "#fff",
                        color: exportEscopo === valor ? "#1d4ed8" : "#334155",
                        cursor: "pointer",
                        fontWeight: 900,
                        textAlign: "left",
                      }}
              ...
              {exportEscopo === "mes" && (
                <div style={{ display: "grid", gap: 8 }}>
                  <label style={{ color: "#334155", fontWeight: 900, fontSize: 13 }}>Mês de instalação</label>
                  <input
                    type="month"
                    value={exportMesInst}
                    onChange={(e) => setExportMesInst(e.target.value)}
                    style={{ ...inputBase, maxWidth: 240 }}
                  />
                </div>
              )}

              {exportEscopo === "periodo" && (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
                  <div style={{ display: "grid", gap: 8 }}>
                    <label style={{ color: "#334155", fontWeight: 900, fontSize: 13 }}>Data inicial</label>
                    <input
                      type="date"
                  ...
                  <div style={{ display: "grid", gap: 8 }}>
                    <label style={{ color: "#334155", fontWeight: 900, fontSize: 13 }}>Data final</label>
                    <input
                      type="date"
                      value={exportDataFim}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: 18 }}>
                <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 16 }}>
                  <strong style={{ display: "block", color: "#0f172a", marginBottom: 12 }}>Abas do arquivo</strong>
                  {([
                    ["clientes", "Clientes"],
                    ["faturas", "Faturas"],
                    ["tabulacao", "Tabulação por mês"],
                    ["filtros", "Resumo dos filtros"],
                  ] as [keyof typeof exportAbas, string][]).map(([campo, label]) => (
                    <label key={campo} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, color: "#334155", fontWeight: 800 }}>
                      <input
                        type="checkbox"
                        checked={exportAbas[campo]}
                        onChange={(e) => setExportAbas((prev) => ({ ...prev, [campo]: e.target.checked }))}
                      />
                      {label}
                    </label>
                  ))}
                ...
                <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 16 }}>
                  <strong style={{ display: "block", color: "#0f172a", marginBottom: 12 }}>Campos</strong>
                  {([
                    ["cliente", "Dados do cliente"],
                    ["cobranca", "Cobrança e faturas"],
                    ["fpd", "Instalação / FPD"],
                  ] as [keyof typeof exportCampos, string][]).map(([campo, label]) => (
                    <label key={campo} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, color: "#334155", fontWeight: 800 }}>
                      <input
                        type="checkbox"
                        checked={exportCampos[campo]}
                        onChange={(e) => setExportCampos((prev) => ({ ...prev, [campo]: e.target.checked }))}
                      />
                      {label}
                    </label>
                  ))}
              ...
              <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 16, padding: 14, color: "#475569", fontSize: 13, fontWeight: 700 }}>
                Clientes encontrados para esta exportação:{" "}
                <span style={{ color: "#2563eb", fontWeight: 900 }}>{clientesParaExportar().length.toLocaleString("pt-BR")}</span>
              </div>
            </div>

            <div style={{ padding: "16px 24px 22px", borderTop: "1px solid #e5e7eb", display: "flex", justifyContent: "flex-end", gap: 10, flexWrap: "wrap" }}>
              <button type="button" onClick={() => setShowExportModal(false)} disabled={exportandoExcel} style={btnSecundario}>
                Cancelar
              </button>
              <button type="button" onClick={exportarExcel} disabled={exportandoExcel} style={{ ...btnPrimario, opacity: exportandoExcel ? 0.65 : 1 }}>
