...
const cores = ["#5b8f74", "#0f766e", "#2563eb", "#7c3aed", "#ca8a04", "#dc2626", "#0891b2"];
const box = { background: "#fff", border: "1px solid #dcebe2", borderRadius: 16, padding: 18, boxShadow: "0 8px 24px rgba(63,98,18,.06)" } as const;
const btn = { border: "1px solid #365f4b", borderRadius: 10, padding: "10px 14px", background: "#5b8f74", color: "#fff", fontWeight: 800, cursor: "pointer" } as const;
...
export function RelatoriosFinanceiros({ titulos }: { titulos: TituloRelatorio[] }) {
  ...
  const imprimirPdf = () => {
    ...
    janela.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Relatório financeiro</title><style>
      @page{size:A4 landscape;margin:12mm}*{box-sizing:border-box}body{font:12px Arial;color:#172033;margin:0}header{border-bottom:3px solid #5b8f74;padding-bottom:12px;margin-bottom:18px}h1{margin:0;font-size:24px}p{color:#64748b}.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.k{border:1px solid #dcebe2;border-radius:10px;padding:12px}.k b{display:block;font-size:17px;margin-top:5px}.chart{margin:18px 0}.mes{display:grid;grid-template-columns:70px 1fr 180px;gap:9px;align-items:center;margin:7px 0}.bars{display:grid;gap:3px}.bars i{height:7px;border-radius:9px}.in{background:#5b8f74}.out{background:#dc2626}table{width:100%;border-collapse:collapse}th,td{padding:7px;border-bottom:1px solid #e2e8f0;text-align:right}th:first-child,td:first-child{text-align:left}.pos{color:#294c3b}.neg{color:#dc2626}footer{margin-top:12px;color:#64748b;font-size:10px}
  ...
  return <div style={{display:"grid",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:10}}>
      {[["Entradas",entradas,"#294c3b"],["Saídas",saidas,"#dc2626"],["Saldo",entradas-saidas,entradas-saidas>=0?"#166534":"#dc2626"],["Vencido em aberto",vencido,"#b45309"]].map(([l,v,c])=><div key={String(l)} style={box}><small style={{color:"#64748b",fontWeight:800}}>{l}</small><b style={{display:"block",fontSize:21,marginTop:8,color:String(c)}}>{brl(Number(v))}</b></div>)}
    </div>
    <div style={{...box,height:360}}><h3>Entradas, saídas e saldo acumulado</h3><ResponsiveContainer width="100%" height="85%"><AreaChart data={mensal}><defs><linearGradient id="gIn" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#5b8f74" stopOpacity={.35}/><stop offset="95%" stopColor="#5b8f74" stopOpacity={.02}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0"/><XAxis dataKey="competencia"/><YAxis tickFormatter={v=>`${Math.round(v/1000)}k`}/><Tooltip formatter={(v:number)=>brl(v)}/><Legend/><Area type="monotone" dataKey="entradas" name="Entradas" stroke="#5b8f74" fill="url(#gIn)"/><Bar dataKey="saidas" name="Saídas" fill="#ef4444" radius={[5,5,0,0]}/><Line type="monotone" dataKey="acumulado" name="Saldo acumulado" stroke="#2563eb" strokeWidth={3}/></AreaChart></ResponsiveContainer></div>
    <div style={{...box}}>
      ...
      <div style={{height:310,marginTop:14}}><ResponsiveContainer width="100%" height="100%"><AreaChart data={cenarios}>
        ...
        <Line type="monotone" dataKey="otimista" name="Otimista" stroke="#5b8f74" strokeWidth={3}/><Line type="monotone" dataKey="realista" name="Realista" stroke="#2563eb" strokeWidth={3}/><Line type="monotone" dataKey="pessimista" name="Pessimista" stroke="#dc2626" strokeWidth={3}/>
      </AreaChart></ResponsiveContainer></div>
    </div>
