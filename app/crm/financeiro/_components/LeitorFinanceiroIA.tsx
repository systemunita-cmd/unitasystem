...
export function LeitorFinanceiroIA() {
  ...
  return <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:16,padding:20,boxShadow:"0 10px 28px rgba(15,23,42,.055)"}}>
    ...
    <div style={{display:"flex",gap:8,alignItems:"center",margin:"12px 0",padding:11,border:"1px solid #dcebe2",borderRadius:11,background:"#f8fbf9"}}><button onClick={testar} disabled={testando} style={{padding:"8px 12px",background:"#0f172a",color:"#fff",border:0,borderRadius:8,fontWeight:700}}>{testando?"Testando...":"Testar configuração da IA"}</button>{diagnostico&&<span style={{fontSize:11,color:diagnostico.status==="operacional"?"#166534":"#b45309",fontWeight:800}}>{diagnostico.status==="operacional"?`Operacional · ${diagnostico.modelo}`:`${diagnostico.status} · ${diagnostico.modelo||""}`}</span>}</div>
    <input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" onChange={e=>setArquivo(e.target.files?.[0]||null)}/>
    <button onClick={ler} disabled={!arquivo||lendo} style={{marginLeft:8,padding:"8px 12px",background:"#5b8f74",color:"#fff",border:0,borderRadius:8}}>{lendo?"Lendo...":"Ler com IA"}</button>
