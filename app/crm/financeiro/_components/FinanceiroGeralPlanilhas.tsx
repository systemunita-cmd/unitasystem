...
type ManualForm = { tipo: "receber" | "pagar"; descricao: string; valor: string; vencimento: string; status: "pendente" | "pago"; categoria: string; centro_custo: string; grupo: "pessoal" | "empresa"; observacao: string };
const manualVazio = (competencia: string): ManualForm => ({ tipo: "pagar", descricao: "", valor: "", vencimento: `${competencia}-01`, status: "pendente", categoria: "", centro_custo: "", grupo: "empresa", observacao: "" });
...
export function FinanceiroGeralPlanilhas() {
  ...
  const salvarManual = async () => {
    ...
    const { error } = await supabase.from("fin_titulos").insert({
      ...
      vencimento: manual.vencimento, pago_em: manual.status === "pago" ? manual.vencimento : null,
  ...
  const renderManual = () => <div className="fg-manual">
    <div className="fg-aviso"><b>Inclusão manual.</b> Use para valores que não vieram do CRM, RH, cobrança ou importação. O lançamento entra nos mesmos totais e relatórios automáticos.</div>
    <div className="fg-form-grid">
      ...
      <label>DESCRIÇÃO *<input value={manual.descricao} onChange={e=>setManual({...manual,descricao:e.target.value})} placeholder="Ex.: aluguel, fornecedor, recebimento" /></label>
      ...
      <label>DATA *<input type="date" value={manual.vencimento} onChange={e=>setManual({...manual,vencimento:e.target.value})} /></label>
      ...
      <label className="fg-form-largo">OBSERVAÇÃO<textarea value={manual.observacao} onChange={e=>setManual({...manual,observacao:e.target.value})} rows={3} /></label>
    </div>
  ...
  return <div className="fg-root">
    ...
    <style jsx global>{`
      .fg-root{display:grid;gap:16px;color:#172033}.fg-hero{background:linear-gradient(135deg,#f7fbf8,#e8f3ec);border:1px solid #c8ddd0;border-radius:22px;padding:24px;color:#173126;display:flex;justify-content:space-between;align-items:center;gap:20px;box-shadow:0 14px 35px rgba(54,95,75,.09)}.fg-hero span{font-size:10px;font-weight:900;letter-spacing:.14em;color:#527965}.fg-hero h1{font-size:25px;margin:6px 0 4px;color:#173126}.fg-hero p{margin:0;color:#60766a;font-size:12px}.fg-acoes{display:flex;align-items:end;gap:8px;flex-wrap:wrap}.fg-acoes label{font-size:9px;font-weight:900;letter-spacing:.08em}.fg-acoes input{display:block;margin-top:5px;min-height:40px;border:1px solid #bfd9ca;border-radius:10px;padding:0 10px;background:#fff;color:#172033}.fg-atualizar,.fg-secundario{min-height:42px;border:0;border-radius:11px;padding:0 16px;font-weight:900;cursor:pointer}.fg-atualizar{background:#5b8f74;color:#fff;border:1px solid #4d7d65}.fg-secundario{background:#fff;color:#365f4b;border:1px solid #c8ddd0}.fg-mensagem{padding:12px 14px;background:#fff7ed;border:1px solid #fdba74;border-radius:12px;color:#9a3412;font-size:12px;display:flex;justify-content:space-between}.fg-mensagem button{border:0;background:transparent;font-size:18px;cursor:pointer}.fg-editor{background:#f3f8f5;border:1px solid #bfd9ca;border-radius:16px;padding:15px;display:grid;grid-template-columns:minmax(220px,1fr) repeat(2,minmax(130px,180px)) auto;gap:10px;align-items:end}.fg-editor small{display:block;color:#64748b;margin-top:4px}.fg-editor label{font-size:10px;font-weight:800;color:#365f4b}.fg-editor input{display:block;width:100%;box-sizing:border-box;margin-top:5px;border:1px solid #cbd5e1;border-radius:9px;padding:10px;background:#fff}.fg-abas{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:9px}.fg-abas button{border:1px solid #d7e4dc;background:#fff;border-radius:14px;padding:11px;text-align:left;display:grid;grid-template-columns:36px 1fr;column-gap:9px;cursor:pointer;box-shadow:0 4px 12px rgba(15,23,42,.05);transition:.18s}.fg-abas button:hover{transform:translateY(-2px);border-color:#7fb095}.fg-abas button.ativo{background:#f3f8f5;border-color:#5b8f74;box-shadow:0 8px 22px rgba(91,143,116,.15)}.fg-abas button>span{grid-row:1/3;width:34px;height:34px;border-radius:10px;display:grid;place-items:center;background:#e6f1eb;color:#365f4b;font-size:10px;font-weight:900}.fg-abas b{font-size:11px;align-self:end}.fg-abas small{color:#64748b;font-size:9px;margin-top:2px}.fg-planilha{background:#fff;border:1px solid #dce7e0;border-radius:19px;overflow:hidden;box-shadow:0 10px 35px rgba(15,23,42,.06)}.fg-planilha>header{padding:16px 18px;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center;background:linear-gradient(90deg,#f8fbf9,#fff)}.fg-planilha>header>div{display:flex;gap:11px;align-items:center}.fg-planilha>header>div>span{width:40px;height:40px;border-radius:12px;display:grid;place-items:center;background:#5b8f74;color:#fff;font-size:11px;font-weight:900}.fg-planilha h2{font-size:16px;margin:0}.fg-planilha header p{margin:3px 0 0;color:#64748b;font-size:10px}.fg-planilha header em{font-style:normal;font-size:9px;font-weight:900;color:#365f4b;background:#e6f1eb;padding:6px 9px;border-radius:999px}.fg-conteudo{padding:17px;display:grid;gap:14px}.fg-manual,.fg-conciliacao{display:grid;gap:15px}.fg-form-grid{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:11px}.fg-form-grid label{font-size:9px;font-weight:900;color:#426451;letter-spacing:.04em}.fg-form-grid input,.fg-form-grid select,.fg-form-grid textarea{display:block;width:100%;box-sizing:border-box;margin-top:6px;border:1px solid #d7e4dc;border-radius:10px;padding:10px;background:#fff;color:#172033;font:inherit;font-size:11px;outline:none}.fg-form-grid input:focus,.fg-form-grid select:focus,.fg-form-grid textarea:focus{border-color:#6f9f84;box-shadow:0 0 0 3px rgba(111,159,132,.14)}.fg-form-largo{grid-column:span 2}.fg-form-acoes{display:flex;justify-content:flex-end;gap:9px}.fg-resumos{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:9px}.fg-resumo{border:1px solid #e2e8f0;border-radius:13px;padding:12px;background:#fff}.fg-resumo small{color:#64748b;display:block;font-size:10px}.fg-resumo strong{display:block;margin-top:4px;font-size:16px}.fg-resumo.verde strong{color:#15803d}.fg-resumo.vermelho strong{color:#b91c1c}.fg-resumo.azul strong{color:#0369a1}.fg-tabela-wrap{overflow:auto;border:1px solid #e2e8f0;border-radius:13px}.fg-tabela{width:100%;border-collapse:collapse;min-width:900px;font-size:10px}.fg-tabela th{background:#f8fafc;color:#64748b;text-transform:uppercase;letter-spacing:.04em;text-align:left;padding:9px;white-space:nowrap}.fg-tabela td{padding:9px;border-top:1px solid #eef2f7;vertical-align:middle}.fg-tabela tbody tr:hover{background:#f8fbf9}.fg-tabela td small{display:block;color:#94a3b8;margin-top:3px}.fg-tabela .numero{font-variant-numeric:tabular-nums;white-space:nowrap}.fg-tabela .total{font-weight:900;color:#365314}.fg-tabela .pendente{color:#b91c1c}.fg-tabela .pago{color:#15803d}.fg-pill{display:inline-flex;border-radius:999px;padding:4px 7px;font-size:9px;font-weight:900;white-space:nowrap}.fg-pill.entrada{background:#dcfce7;color:#166534}.fg-pill.saida{background:#fee2e2;color:#991b1b}.fg-link{border:0;background:transparent;color:#365f4b;text-decoration:underline;font-size:9px;font-weight:800;cursor:pointer}.fg-vazio{border:1px dashed #cbd5e1;border-radius:13px;padding:30px;text-align:center;color:#94a3b8}.fg-aviso{background:#f8fafc;border-left:4px solid #5b8f74;border-radius:9px;padding:11px;color:#475569;font-size:10px}.fg-aviso.sucesso{background:#f0fdf4;color:#166534}.fg-calculo-btn{border:1px solid #7fb095;background:#f3f8f5;color:#365314;border-radius:8px;padding:6px 9px;font-size:9px;font-weight:900;cursor:pointer;white-space:nowrap}.fg-detalhe{border:1px solid #bfd9ca;background:#f8fbf9;border-radius:15px;padding:15px}.fg-detalhe>header{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}.fg-detalhe>header small{display:block;color:#64748b;margin-top:3px}.fg-detalhe>header button{border:0;background:transparent;font-size:20px;cursor:pointer}.fg-detalhe-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:8px}.fg-detalhe-grid>div{background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:9px}.fg-detalhe-grid small{display:block;color:#64748b;font-size:9px}.fg-detalhe-grid b{display:block;margin-top:4px;font-size:11px;color:#365314}.fg-detalhe-grid b.negativo{color:#b91c1c}.fg-detalhe>p{font-size:10px;color:#475569;margin:12px 0 0}@media(max-width:900px){.fg-form-grid{grid-template-columns:1fr 1fr}.fg-hero{align-items:flex-start;flex-direction:column}.fg-editor{grid-template-columns:1fr 1fr}.fg-editor>div{grid-column:1/-1}}@media(max-width:560px){.fg-form-grid{grid-template-columns:1fr}.fg-form-largo{grid-column:auto}.fg-hero{padding:18px}.fg-hero h1{font-size:20px}.fg-acoes{width:100%}.fg-acoes label{width:100%}.fg-acoes input{width:100%;box-sizing:border-box}.fg-editor{grid-template-columns:1fr}.fg-editor>div{grid-column:auto}.fg-abas{grid-template-columns:1fr 1fr}.fg-planilha>header em{display:none}.fg-conteudo{padding:11px}}
    `}</style>
  </div>;
}
