...
// ═══════════════════════════════════════════════════════════════════════
// 💵 Financeiro · Contas a Receber (CONECTADO — fin_titulos tipo='receber')
...
export function ContasReceberSection() {
  ...
  const erro = (titulo: string, e?: any) => {
    if (e) console.error("[ContasReceber]", e);
  ...
  const carregar = async () => {
    ...
    if (error) {
      erro(msgAmigavel(error, "Não consegui carregar os títulos."), error);
      setCarregando(false);
      return;
    }
    setLista(
  ...
  const salvar = async () => {
    if (!form.descricao.trim()) {
      erro("Informe a descrição do título.");
      return;
    }
    setSalvando(true);
    const payload = {
    ...
    setSalvando(false);
    if (error) {
      erro(msgAmigavel(error, "Não consegui salvar o título."), error);
      return;
    }
    setModal(false);
    setForm(FORM_VAZIO);
    ok(form.id ? "Título atualizado." : "Título criado.");
    carregar();
  };
  ...
  const excluir = async (t: Titulo) => {
    if (!confirm(`Excluir o título "${t.descricao}"?`)) return;
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ color: "#6b7280", fontSize: 12, margin: "2px 0 0" }}>
              Títulos, vencimentos e recebimentos
            </p>
          </div>
        </div>
        <button
      ...
      {carregando ? (
        ...
        <div style={{ ...card, padding: 40, textAlign: "center" }}>
          ...
          <p style={{ color: "#6b7280", fontSize: 13 }}>
            {lista.length === 0 ? "Nenhum título a receber ainda." : "Nada com esse filtro."}
          </p>
        </div>
      ) : (
          ...
          >
            ...
            >
              <h3 style={{ color: "#1f2937", fontSize: 16, fontWeight: 700, margin: 0 }}>
                {form.id ? "Editar título" : "Novo título a receber"}
              </h3>
              <button
                onClick={() => setModal(false)}
                style={{
