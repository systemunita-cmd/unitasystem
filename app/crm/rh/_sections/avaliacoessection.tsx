...
// 🧑‍💼 RH · Avaliações de Desempenho (CONECTADO — 'avaliacoes')
const COR = "#4f46e5";
...
export function AvaliacoesSection() {
    ...
    if (error) {
      ...
      setLista(
        (data || []).map((r: any) => ({
          ...
          ciclo: r.ciclo || "",
          nota: r.nota || 0,
          status: (r.status || "pendente") as StatusA,
        }))
      );
    setCarregando(false);
  };
  ...
  const stats = useMemo(() => {
    const c = lista.filter((a) => a.status === "concluida");
    return {
      pendentes: lista.filter((a) => a.status === "pendente").length,
      concluidas: c.length,
      media: c.length ? Math.round(c.reduce((s, a) => s + a.nota, 0) / c.length) : 0,
    };
  }, [lista]);
  const salvar = async () => {
    if (!form.nome.trim()) {
  ...
  const excluir = async (a: Avaliacao) => {
    if (!confirm(`Remover avaliação de ${a.nome}?`)) return;
    const { error } = await supabase.from("avaliacoes").delete().eq("id", a.id);
    if (error) {
      alert("Erro: " + error.message);
      return;
    }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ color: "#6b7280", fontSize: 12, margin: "2px 0 0" }}>
              Ciclos de avaliação e notas dos colaboradores
            </p>
          </div>
        </div>
        <button
      ...
      {carregando ? (
        ...
        <div style={{ ...card, padding: 40, textAlign: "center" }}>
          ...
          <p style={{ color: "#6b7280", fontSize: 13 }}>Nenhuma avaliação cadastrada.</p>
        </div>
      ) : (
                ...
                {lista.map((a, i) => (
                  ...
                  >
                          ...
                          >
                            <div
                              style={{
                                height: "100%",
                                width: `${a.nota}%`,
                                background: corNota(a.nota),
                                borderRadius: 5,
                              }}
                            />
                          </div>
                          <span style={{ color: corNota(a.nota), fontSize: 13, fontWeight: 800 }}>
                            {a.nota}
                          </span>
                        </div>
                      )}
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <span
                        style={{
                          background: a.status === "concluida" ? "#f0fdf4" : "#fffbeb",
                          color: a.status === "concluida" ? "#16a34a" : "#f59e0b",
                          border: `1px solid ${a.status === "concluida" ? "#bbf7d0" : "#fde68a"}`,
                          fontSize: 11,
                          padding: "3px 10px",
                          borderRadius: 10,
                          fontWeight: 700,
        ...
        >
          ...
          >
            ...
            <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <Campo label="Nota (0–100)">
                  <input
                    type="number"
                    value={form.nota || ""}
                    onChange={(e) => set("nota", Number(e.target.value))}
                    style={inputStyle}
                  />
                </Campo>
                <Campo label="Status">
                  <select
                    value={form.status}
                    onChange={(e) => set("status", e.target.value as StatusA)}
                    style={inputStyle}
                  >
