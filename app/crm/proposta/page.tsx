...
//   • 🛡️ Escopo (own/team/all) + filtro por equipe aplicados no CLIENT,
//     usando equipe_id_criador da proposta (robusto contra timing de carga).
// ═══════════════════════════════════════════════════════════════════════════
...
export default function Vendas() {
  ...
  // 👥 Filtro por equipe (Diretor travado na dele; admin escolhe)
  ...
  const renderFiltroColuna = (c: CampoUnificado): ReactNode => {
    ...
    if (c.slug === "vendedor" || c.tipo === "vendedor") {
      const vendedoresVisiveis = equipeId
        ? usuarios.filter(u => String(u.equipe_id ?? "") === String(equipeId))
        : usuarios;
      return (
  ...
  // Traz tudo que o banco permite (RLS quando ligado dá a segurança real).
  // O recorte por escopo/equipe é feito no client (propostasFiltradas),
  // evitando problema de timing de carga das permissões.
  const fetchPropostas = async (): Promise<boolean> => {
    ...
    try {
      while (offset < TOTAL_LIMITE) {
        const q = supabase.from("proposta").select("*")
  ...
  const propostasFiltradas = useMemo(() => propostas
    // 🛡️ ESCOPO: own = só minhas; team = só da minha equipe; all/admin = todas.
    .filter(p => {
      if (perm.superAdmin || podeVerTudo || escopoVer === "all") return true;
      if (escopoVer === "own") {
        const meu = (perm.userEmail || userEmail || "").toLowerCase();
        return (p.vendedor || "").toLowerCase() === meu
          || String(p.criado_por ?? "").toLowerCase() === meu;
      }
      // escopo "team" → o recorte vem do filtro de equipe abaixo (equipeId travado)
      return true;
    })
    // 👥 EQUIPE: Diretor travado na dele OU admin que escolheu uma. Sem equipe = todas.
    .filter(p => {
      if (!equipeId) return true;
      return String(p.equipe_id_criador ?? "") === String(equipeId);
    })
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [propostas, perm.superAdmin, perm.userEmail, escopoVer, podeVerTudo, userEmail, equipeId, filtroStatus, busca, filtroDataInicio, filtroDataFim, filtrosColuna, usuarios, camposUnificados]
  );
          ...
          <div>
            ...
            <p style={{ color: "#6b7280", fontSize: 12, margin: "2px 0 0" }}>
              {podeVerTudo
                ...
                : <><b style={{ color: "#2563eb" }}>{totalVisivel}</b> proposta(s) visível(is)</>}
            </p>
          </div>
        </div>
              ...
              {loading ? (
                ...
                <tr><td colSpan={colunasTabela.length + 1} style={{ padding: 48, textAlign: "center" }}>
                  ...
                  <p style={{ color: "#6b7280", fontSize: 13, margin: 0 }}>
                    {busca || filtroStatus !== "todos" ? "Nenhum resultado pros filtros" : podeVerTudo ? "Nenhuma proposta cadastrada ainda" : "Nenhuma proposta visível pra você"}
                  </p>
                </td></tr>
