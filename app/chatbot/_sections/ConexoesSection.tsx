...
const UNITAZAP_URL =
  process.env.NEXT_PUBLIC_UNITAZAP_URL || "http://localhost:3001";
...
type Conexao = {
  ...
  api_key?: string;
  prompt: string;
  parar_se_atendente: boolean;
  phone_number_id?: string;
  waba_id?: string;
  token_waba?: string;
  webhook_token?: string;
  typebot_url?: string;
  typebot_msg_invalida?: string;
  typebot_msg_boas_vindas?: string;
...
export function ConexoesSection() {
  ...
  const ehAdminGeralCon =
    perm.superAdmin || perm.grupoNome === "Administração Geral";

  const equipeForcadaCon =
    !perm.carregando && !ehAdminGeralCon && perm.equipeId != null
  ...
  const [showMenuEngrenagem, setShowMenuEngrenagem] = useState<number | null>(
    null
  );
  ...
  const [resultadoMeta, setResultadoMeta] = useState<{
    sucesso?: boolean;
    mensagem?: string;
    pages?: any[];
  } | null>(null);
  ...
  const [pagesSelecionadas, setPagesSelecionadas] = useState<Set<string>>(
    new Set()
  );
  ...
  const [wabaTeste, setWabaTeste] = useState<{
    success: boolean;
    nome?: string;
    error?: string;
  } | null>(null);
  ...
  const formInicial = {
    ...
    equipeId: "",
    fila: "",
    pararSeAtendente: true,
    typebot_url: "",
    typebot_msg_invalida: "",
    typebot_msg_boas_vindas: "",
  };
  ...
  const equipeIdInicial =
    travadoEquipe && equipeForcadaCon ? equipeForcadaCon : "";

  const nomeEquipeForcada =
    equipes.find((e) => String(e.id) === equipeForcadaCon)?.nome ||
    "Minha equipe";

  const novoForm = () => ({
    ...formInicial,
    equipeId: equipeIdInicial,
  });

  useEffect(() => {
    if (!FB_APP_ID || window.FB || document.getElementById("facebook-jssdk")) {
      return;
    }
  ...
  const verificarStatusWaba = async (canalId: number) => {
    try {
      const resp = await fetch(
        `${UNITAZAP_URL}/waba/verificar-status?canalId=${canalId}`
      );
  ...
  useEffect(() => {
    ...
    const ch = supabase
      .channel("conexoes_rt_unita")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "conexoes" },
        () => fetchConexoes()
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "filas" },
        () => fetchFilas()
      )
      .on(
        "postgres_changes",
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.sessoes && Array.isArray(data.sessoes)) {
          const { data: canaisBanco } = await supabase
            .from("conexoes")
            .select("*");
          ...
          for (const c of canaisBanco) {
            if (c.tipo === "webjs") {
              const sessaoVPS = data.sessoes.find(
                (s: any) => s.canalId === c.id
              );

              if (!sessaoVPS) continue;

              const statusReal =
                sessaoVPS.status === "conectado"
                  ? "conectado"
                  : "desconectado";
              ...
              if (
                c.status !== statusReal ||
                (statusReal === "conectado" && c.numero !== numeroReal)
              ) {
                await supabase
                  .from("conexoes")
                  .update({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        const resp = await fetch(
          `${UNITAZAP_URL}/qr-data?canalId=${qrCanalId}`,
          { cache: "no-store" }
        );

        if (!resp.ok) {
          console.warn(
            `[QR poll] status HTTP ${resp.status} — tentativa ${tentativas}`
          );
          return;
        }
  ...
  }, [qrPolling, showModalQR, qrCanalId, qrImageUrl]);
  ...
  const registrarNumeroWaba = async (c: Conexao) => {
    ...
    if (!usarPinPadrao) {
      ...
      if (!/^\d{6}$/.test(pinCustom)) {
        notify(
          "PIN deve ter exatamente 6 dígitos",
          "aviso",
          "Digite os 6 números do seu PIN de 2 fatores"
        );
        return;
      }
    ...
    try {
      ...
      if (data.success) {
        notify(
          ...
          "sucesso",
          "Agora está ONLINE e pode receber mensagens"
        );
        await fetchConexoes();
      } else {
        ...
        notify(
          traduzirErro({ error: mensagemErro, codigo }),
          "erro",
          dica.replace(/^\n\n💡 ?/, "") || undefined
        );
      }
    } catch (e: any) {
      notify("Operação falhou", "erro", traduzirErro(e));
    }

    setRegistrandoWaba(false);
  };

  const encerrarAtendimentosEmMassa = async (
    tipo: "aguardando" | "abertos",
    c: Conexao
  ) => {
    const statusAlvo =
      tipo === "aguardando" ? ["pendente"] : ["aberto", "em_atendimento"];
    ...
    if (errBusca) {
      notify("Erro ao buscar atendimentos", "erro", traduzirErro(errBusca));
      return;
    }
    ...
    if (total === 0) {
      notify(`Não há atendimentos ${labelTipo} em "${c.nome}"`, "info");
      setShowMenuEngrenagem(null);
      return;
    }
    ...
    try {
      ...
      const mensagensSistema = (atendimentos || []).map((a) => ({
        numero: a.numero,
        mensagem: `Chat encerrado em massa (${labelTipo}) por: Sistema`,
        de: "sistema",
        canal_id: c.id,
      }));
  ...
  const testarWABA = async () => {
    if (!form.phoneNumberId || !form.token) {
      notify("Preencha Phone Number ID e Token", "aviso");
      return;
    }

    setTestandoWABA(true);
    setWabaTeste(null);
  ...
  const abrirEditar = (c: Conexao) => {
    ...
    setForm({
      ...
      apiKey: "",
      prompt: c.prompt || "",
      fluxoId: c.fluxo_id ? String(c.fluxo_id) : "",
      equipeId: equipeDaFila
        ? String(equipeDaFila)
        : travadoEquipe && equipeForcadaCon
        ? equipeForcadaCon
        : "",
      fila: c.fila || "",
      pararSeAtendente: c.parar_se_atendente,
      typebot_url: c.typebot_url || "",
      typebot_msg_invalida: c.typebot_msg_invalida || "",
  ...
  const toggleMetaFlag = async (
    canal: any,
    flag: "instagram_ativo" | "messenger_ativo"
  ) => {
    try {
  ...
  const conectarMeta = () => {
    if (!FB_APP_ID || !FB_CONFIG_ID) {
      notify(
        ...
        "aviso",
        "Configure NEXT_PUBLIC_FB_APP_ID e NEXT_PUBLIC_FB_CONFIG_ID no .env"
      );
      return;
    }
    ...
    if (!window.FB) {
      notify(
        "Sistema do Facebook carregando",
        "aviso",
    ...
    window.FB.login(
      (response: any) => {
        if (!response.authResponse) {
          ...
          setResultadoMeta({
            sucesso: false,
            mensagem: "Você cancelou a conexão.",
          });
          return;
        }
  ...
  const confirmarSelecaoPages = async () => {
    ...
    const pagesEscolhidas = pagesDisponiveis.filter((p) =>
      pagesSelecionadas.has(p.id)
    );
  ...
  const salvarCanal = async () => {
    if (!editandoId && form.tipo === "meta_oauth") {
      notify(
        "Canal Meta já conectado",
        "info",
        "Clique em 'Concluir' pra fechar este modal"
      );
      return;
    }
    ...
    if (!isDono) {
      notify(
        "Sem permissão",
        "erro",
        "Apenas administradores podem gerenciar canais"
      );
      return;
    }

    if (!form.nome.trim()) {
      notify("Digite o nome do canal", "aviso");
      return;
    }
    ...
    if (!form.fila) {
      notify(
        "Selecione uma fila",
        "aviso",
        "Se não tem fila cadastrada, vá em Configurações → Filas"
      );
      return;
    }

    if (!editandoId && form.tipo === "waba" && (!form.phoneNumberId || !form.token)) {
      notify("Preencha Phone Number ID e Token", "aviso");
      return;
    }

    if (!editandoId && form.modo === "ia" && !form.apiKey) {
      notify("Digite a API Key da IA", "aviso");
      return;
    }
    ...
    try {
      const fluxoSel =
        form.modo === "fluxo"
          ? fluxos.find((f) => String(f.id) === String(form.fluxoId))
          : undefined;

      const fluxoIdTratado =
        form.modo === "fluxo" && form.fluxoId
          ? Number(form.fluxoId)
          : null;

      if (form.modo === "fluxo" && !fluxoIdTratado) {
        notify("Selecione um fluxo", "aviso");
      ...
      const payload: any = {
        ...
        typebot_msg_invalida:
          form.typebot_msg_invalida ||
          "Desculpe, não entendi sua resposta. Pode tentar de novo?",
        typebot_msg_boas_vindas: form.typebot_msg_boas_vindas || "",
      };

      if (apiKeyTocada || !editandoId) {
        payload.api_key = form.modo === "ia" ? form.apiKey || "" : "";
      }

      console.log("Payload para salvar canal:", payload);
      ...
      if (editandoId) {
        ...
        if (updateErr) {
          console.error("Erro Supabase ao atualizar canal:", updateErr);
          throw updateErr;
        }
        ...
        try {
          ...
          console.warn("Aviso: canal atualizado, mas falhou configurar IA:", e);
        }
        ...
        if (form.tipo === "waba") {
          ...
          if (insErr) {
            console.error("Erro Supabase ao salvar canal WABA:", insErr);
            throw insErr;
          }

          novoId = inserted.id;
        } else {
          const insertWebjs: any = {
            tipo: "webjs",
            status: "desconectado",
            numero: "",
            ...payload,
          ...
          console.log("Insert WebJS:", insertWebjs);
          ...
          if (insErr) {
            console.error("Erro Supabase ao salvar canal WebJS:", insErr);
            throw insErr;
          }
        ...
        if (novoId) {
          try {
            const respCriar = await wa("canal/criar", {
              canalId: novoId,
            });

            console.log("Resposta UnitaZAP canal/criar:", respCriar);
          } catch (e) {
            console.error("Erro ao criar sessão no UnitaZAP:", e);
            notify(
              "Canal salvo, mas sessão não foi criada",
              "aviso",
              "O canal ficou no banco, mas o UnitaZAP não respondeu. Tente reconectar pelo botão do canal."
            );
          }
        }
      ...
      console.error("ERRO COMPLETO AO SALVAR CANAL:", e);
      notify("Operação falhou", "erro", traduzirErro(e));
    } finally {
      setSalvandoCanal(false);
    }
  ...
  const abrirQR = async (id: number) => {
    ...
    try {
      ...
      console.error("Erro ao resetar canal:", e);
    }
  ...
  const reconectarCanal = async (c: Conexao) => {
    ...
    try {
      const data = await wa("reconectar", {
        canalId: c.id,
      });
      ...
      if (data.sessao_salva === false) {
        ...
        notify(
          `${c.nome} reconectando...`,
          "sucesso",
          "O login será restaurado automaticamente. Não precisa escanear QR"
        );
      }
    } catch (e: any) {
  ...
  const desconectarCanal = async (c: Conexao) => {
    if (!confirm(`Desconectar ${c.nome}? Isso vai desconectar o WhatsApp.`)) {
      return;
    }
  ...
  const excluirCanal = async (id: number) => {
    if (!isDono) {
      notify(
        "Sem permissão",
        "erro",
        "Apenas administradores podem excluir canais"
      );
      return;
    }
    ...
    if (
      !confirm(
        "Excluir esse canal?\n\nTodo o histórico vai ser preservado mas o canal será removido."
      )
    ) {
      return;
    }
    ...
    if (!canal) {
      notify("Canal não encontrado", "erro");
      return;
    }
    ...
    if (error) {
      notify("Erro ao excluir canal", "erro", traduzirErro(error));
      return;
    }
  ...
  const iaLabel: Record<string, string> = {
    gpt: "ChatGPT",
    claude: "Claude AI",
    gemini: "Gemini",
    deepseek: "DeepSeek",
  ...
  const filasFiltradas = filasBanco.filter(
    (f) => !form.equipeId || String(f.equipe_id || "") === form.equipeId
  );
    ...
    return (
      <div style={{ padding: 32, textAlign: "center" }}>
        ...
        <p style={{ color: "#9ca3af", fontSize: 12 }}>
          Grupo: <b>{perm.grupoNome || "(sem grupo)"}</b>
        </p>
      </div>
    );
  }
    ...
    >
          ...
          <div style={{ ...cardStyle, padding: 32, width: 420, textAlign: "center" }}>
            ...
            {qrPolling && !qrConectado && qrTentativas >= 20 && (
              ...
              >
                ...
                >
                  ⚠️ Tá demorando mais que o normal
                </p>
                ...
                >
                  Se já aparece conectado no celular, clica em <b>Já Conectei!</b>{" "}
                  pra atualizar. Senão, tenta gerar um novo QR.
                </p>
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (data.status === "conectado") {
                        ...
                        notify(
                          "Backend ainda não reconheceu a conexão",
                          "aviso",
                          `Status atual: ${data.status}. Tenta de novo ou recria o QR.`
                        );
                      }
                    } catch (e: any) {
                      notify("Falha ao verificar QR", "erro", traduzirErro(e));
                    }
                  }}
                  style={{
                    background: "#f59e0b",
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {editandoId ? "✏️ Editar Canal" : "➕ Novo Canal"}
                </h2>
                <p style={{ color: "#6b7280", fontSize: 12, margin: "4px 0 0" }}>
                  {editandoId
                    ? "Altere as configurações"
                    : `${conexoesVisiveis.length} canais cadastrados`}
                </p>
              </div>

              <button
                onClick={fecharModalNovoCanal}
                style={{
                  background: "#f3f4f6",
                  border: "none",
                  color: "#6b7280",
            ...
            >
                  ...
                  >
                    {[
                      ...
                      <button
                        ...
                        style={{
                          background:
                            form.tipo === t.key ? `${t.cor}10` : "#f9fafb",
                          border: `2px solid ${
                            form.tipo === t.key ? t.cor : "#e5e7eb"
                          }`,
                          borderRadius: 12,
                          padding: "14px 16px",
                          cursor: "pointer",
                      ...
                      >
                        ...
                        <p
                          style={{
                            color: form.tipo === t.key ? t.cor : "#1f2937",
                            fontSize: 13,
                            fontWeight: 700,
                            margin: "0 0 2px",
                          }}
              ...
              {editandoId &&
                (form.tipo === "instagram" || form.tipo === "messenger") && (
                  <div
                    style={{
                      background: "#f0fdf4",
                      borderRadius: 12,
                  ...
                  >
                    ...
                    >
                      {form.tipo === "instagram"
                        ? "📷 Canal Instagram"
                        : "💬 Canal Messenger"}{" "}
                      conectado via Login com Facebook.
                    </p>
                    <p
                      style={{
                        color: "#6b7280",
                    ...
                    >
                      Pra reconectar, delete este canal e crie um novo via
                      "Facebook / Instagram".
                    </p>
                  </div>
                )}
              ...
              {!editandoId && form.tipo === "meta_oauth" && (
                <div>
                  ...
                  >
                    ...
                    >
                      Vamos conectar todas as suas <b>fan pages do Facebook</b> e
                      respectivas contas do <b>Instagram Business</b>{" "}
                      automaticamente.
                    </p>
                    ...
                    >
                      Você logará no Facebook, autorizará as permissões necessárias
                      e o sistema criará os canais sozinho.
                    </p>
                    ...
                    {resultadoMeta && (
                      <div
                        style={{
                          background: resultadoMeta.sucesso ? "#f0fdf4" : "#fef2f2",
                          border: `1px solid ${
                            resultadoMeta.sucesso ? "#bbf7d0" : "#fecaca"
                          }`,
                          borderRadius: 10,
                          padding: 14,
                        }}
                      >
                        <p
                          style={{
                            color: resultadoMeta.sucesso ? "#15803d" : "#dc2626",
                            fontSize: 13,
                            margin: "0 0 6px",
                            fontWeight: 700,
                          }}
                        >
                          {resultadoMeta.sucesso ? "✅ " : "❌ "}
                          {resultadoMeta.mensagem}
                        </p>
              ...
              {form.tipo === "waba" && (
                <div>
                  ...
                  >
                    {editandoId ? "2" : "3"}. Credenciais da API Meta
                  </p>
                  ...
                  >
                    ...
                    <button
                      onClick={testarWABA}
                      disabled={testandoWABA}
                      style={{
                        background: testandoWABA ? "#1e40af" : "#2563eb15",
                        color: "#2563eb",
                    ...
                    {wabaTeste && (
                      <div
                        style={{
                          background: wabaTeste.success ? "#f0fdf4" : "#fef2f2",
                          border: `1px solid ${
                            wabaTeste.success ? "#bbf7d0" : "#fecaca"
                          }`,
                          borderRadius: 10,
                          padding: 12,
                        }}
                      >
                        <p
                          style={{
                            color: wabaTeste.success ? "#15803d" : "#dc2626",
                            fontSize: 13,
                            margin: 0,
                            fontWeight: 700,
                          }}
              ...
              <div>
                ...
                >
                  {editandoId ? "2" : form.tipo === "waba" ? "4" : "3"}.
                  Automação
                </p>
                ...
                >
                  {[
                    ...
                    <button
                      ...
                      onClick={() =>
                        setForm((p) => ({
                          ...p,
                          modo: m.key,
                          fluxoId: m.key === "fluxo" ? p.fluxoId : "",
                        }))
                      }
                      style={{
                        background:
                          form.modo === m.key ? `${m.cor}10` : "#f9fafb",
                        border: `2px solid ${
                          form.modo === m.key ? m.cor : "#e5e7eb"
                        }`,
                        borderRadius: 12,
                        padding: "12px 10px",
                        cursor: "pointer",
                    ...
                    >
                      ...
                      <p
                        style={{
                          color: form.modo === m.key ? m.cor : "#1f2937",
                          fontSize: 12,
                          fontWeight: 700,
                          margin: "0 0 2px",
                        }}
                  ...
                  >
                      ...
                      {[
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setApiKeyTocada(true);
                          }}
                          style={{
                            background:
                              form.ia === ia.key ? `${ia.cor}10` : "#ffffff",
                            border: `2px solid ${
                              form.ia === ia.key ? ia.cor : "#e5e7eb"
                            }`,
                            borderRadius: 10,
                            padding: "10px 12px",
                            cursor: "pointer",
                        ...
                        >
                          <p
                            style={{
                              color: form.ia === ia.key ? ia.cor : "#1f2937",
                              fontSize: 13,
                              fontWeight: 700,
                              margin: "0 0 2px",
                            }}
                    ...
                    <div>
                      ...
                      <input
                        type="password"
                        placeholder={
                          editandoId ? "Deixe vazio pra manter" : "Cole sua API Key"
                        }
                        value={form.apiKey}
                        onChange={(e) => {
                          setForm((p) => ({
                ...
                {form.modo === "typebot" && (
                  ...
                  >
                    ...
                    >
                      Cole a URL de publicação do seu Typebot.
                    </p>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="https://typebot.io/meu-bot"
                        value={form.typebot_url || ""}
                        onChange={(e) =>
                          setForm((p) => ({
                            ...p,
                            typebot_url: e.target.value,
                          }))
                        }
                        style={IS}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setForm((p) => ({
                            ...p,
                            typebot_msg_boas_vindas: e.target.value,
                          }))
                        }
                        style={IS}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setForm((p) => ({
                            ...p,
                            typebot_msg_invalida: e.target.value,
                          }))
                        }
                        style={IS}
                      />
              ...
              <div>
                ...
                >
                  {editandoId ? "3" : form.tipo === "waba" ? "5" : "4"}. Equipe &
                  Fila / Departamento
                </p>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setForm((p) => {
                          const filaAtual = filasBanco.find(
                            (f) => f.nome === p.fila
                          );

                          const filaContinuaValida =
                            !novaEquipe ||
                            (filaAtual &&
                              String(filaAtual.equipe_id || "") === novaEquipe);

                          return {
                            ...p,
                            equipeId: novaEquipe,
                            fila: filaContinuaValida ? p.fila : "",
                          };
                        });
                      }}
                      style={IS}
                ...
                {filasBanco.length === 0 ? (
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ color: "#78350f", fontSize: 11, margin: 0 }}>
                        Crie filas em <b>Configurações → Filas</b> antes de criar o
                        canal.
                      </p>
                    </div>
                  ...
                  <>
                    ...
                    {form.equipeId && filasFiltradas.length === 0 && (
                      ...
                      >
                        ⚠️ Essa equipe não tem nenhuma fila cadastrada.
                      </p>
                    )}
                  </>
                )}
                ...
                <p style={{ color: "#9ca3af", fontSize: 11, margin: "6px 0 0" }}>
                  Filas e equipes são gerenciadas em <b>Configurações → Filas</b>.
                </p>
              </div>
              ...
              <div>
                ...
                >
                  {editandoId ? "4" : form.tipo === "waba" ? "6" : "5"}.
                  Comportamento
                </p>
                ...
                >
                  <div>
                    ...
                    >
                      🛑 Parar automação quando atendente assumir
                    </p>
                    <p style={{ color: "#6b7280", fontSize: 11, margin: "4px 0 0" }}>
                      A IA e o fluxo param automaticamente
                    </p>
                  </div>

                  <Toggle
                    value={form.pararSeAtendente}
                    onChange={() =>
                      setForm((p) => ({
                        ...p,
                        pararSeAtendente: !p.pararSeAtendente,
                      }))
                    }
                  />
                </div>
              ...
              {!editandoId && form.tipo === "meta_oauth" ? (
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: "10px 28px",
                    fontSize: 13,
                    cursor: salvandoCanal ? "wait" : "pointer",
                    fontWeight: 700,
                    boxShadow: "0 4px 12px rgba(22,163,74,0.3)",
                  }}
                >
          ...
          <div>
            ...
            <p style={{ color: "#6b7280", fontSize: 13, margin: "2px 0 0" }}>
              ...
              {travadoEquipe && (
                <>
                  {" "}
                  · <b style={{ color: "#7c3aed" }}>👥 {nomeEquipeForcada}</b>
                </>
              )}
            </p>
          </div>
      ...
      {conexoesVisiveis.length === 0 ? (
        <div style={{ ...cardStyle, padding: 48, textAlign: "center" }}>
          ...
          <p style={{ color: "#6b7280", fontSize: 13, margin: "0 0 20px" }}>
            Crie seu primeiro canal pra começar
          </p>
          ...
          {conexoesVisiveis.map((c) => (
            <div
              ...
              style={{
                ...
                padding: 24,
                borderTop: `3px solid ${
                  c.status === "conectado" ? "#16a34a" : "#ef4444"
                }`,
              }}
              onMouseEnter={(e) => {
            ...
            >
              ...
              >
                ...
                <span
                  style={{
                    background:
                      c.status === "conectado" ? "#f0fdf4" : "#fef2f2",
                    color: c.status === "conectado" ? "#16a34a" : "#dc2626",
                    border: `1px solid ${
                      c.status === "conectado" ? "#bbf7d0" : "#fecaca"
                    }`,
                    fontSize: 11,
                    padding: "4px 10px",
                    borderRadius: 20,
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  <span
                    style={{
                      color: modoColor[c.modo] || "#6b7280",
                      fontSize: 12,
                      fontWeight: 700,
                    }}
                  >
                    {c.modo === "ia"
                      ? `🤖 IA (${iaLabel[c.ia] || c.ia})`
                      : c.modo === "fluxo"
                      ? `🔀 ${c.fluxo_nome}`
                      : c.modo === "typebot"
                      ? `🎯 Typebot`
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "#6b7280", fontSize: 12 }}>Fila:</span>
                  <span
                    style={{
                      color: "#2563eb",
                      fontSize: 12,
                ...
                {(() => {
                  const eqId = filasBanco.find((f) => f.nome === c.fila)
                    ?.equipe_id;

                  const eqNome = eqId
                    ? equipes.find((e) => e.id === eqId)?.nome
                    : null;
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {c.tipo === "webjs" &&
                  (c.status === "desconectado" ? (
                    <>
                      <button
                        onClick={() => reconectarCanal(c)}
                        title="Tenta reconectar SEM apagar o login"
                        style={{
                          flex: 1,
                          background:
                            "linear-gradient(135deg, #16a34a 0%, #22c55e 100%)",
                      ...
                      <button
                        onClick={() => abrirQR(c.id)}
                        title="Apaga sessão salva e gera QR novo"
                        style={{
                          background: "#ffffff",
                          color: "#6b7280",
                          border: "1px solid #e5e7eb",
                    ...
                    <>
                      <button
                        onClick={() => reconectarCanal(c)}
                        title="Reconectar caso esteja com erro"
                        style={{
                          flex: 1,
                          background: "#f0fdf4",
                          color: "#16a34a",
                ...
                {c.tipo === "meta" && (
                  <div style={{ flex: 1, display: "flex", gap: 6 }}>
                    <button
                      ...
                      title={c.messenger_ativo ? "Messenger ligado" : "Messenger desligado"}
                      style={{
                        flex: 1,
                        background: c.messenger_ativo ? "#1877f2" : "#f9fafb",
                        color: c.messenger_ativo ? "white" : "#6b7280",
                        border: `1px solid ${
                          c.messenger_ativo ? "#1877f2" : "#e5e7eb"
                        }`,
                        borderRadius: 10,
                        padding: "9px 4px",
                        fontSize: 13,
                    ...
                    {c.instagram_business_id ? (
                      <button
                        ...
                        title={
                          c.instagram_ativo ? "Instagram ligado" : "Instagram desligado"
                        }
                        style={{
                          flex: 1,
                          background: c.instagram_ativo ? "#e1306c" : "#f9fafb",
                          color: c.instagram_ativo ? "white" : "#6b7280",
                          border: `1px solid ${
                            c.instagram_ativo ? "#e1306c" : "#e5e7eb"
                          }`,
                          borderRadius: 10,
                          padding: "9px 4px",
                          fontSize: 13,
                ...
                <div style={{ position: "relative" }}>
                  <button
                    onClick={() =>
                      setShowMenuEngrenagem(
                        showMenuEngrenagem === c.id ? null : c.id
                      )
                    }
                    disabled={encerrandoMassa || registrandoWaba}
                    style={{
                      background: "#f9fafb",
                      color: "#6b7280",
                      border: "1px solid #e5e7eb",
                      ...
                      fontSize: 14,
                      cursor:
                        encerrandoMassa || registrandoWaba ? "wait" : "pointer",
                      transition: "all 0.15s",
                    }}
                  >
                  ...
                  {showMenuEngrenagem === c.id && (
                    ...
                    >
                      ...
                      <button
                        onClick={() => encerrarAtendimentosEmMassa("aguardando", c)}
                        style={{
                          display: "block",
                          width: "100%",
                          background: "none",
                      ...
                      <button
                        onClick={() => encerrarAtendimentosEmMassa("abertos", c)}
                        style={{
                          display: "block",
                          width: "100%",
                          background: "none",
        ...
        >
          ...
          >
            <div style={{ padding: "20px 28px", borderBottom: "1px solid #e5e7eb" }}>
              ...
              >
                📲 Escolha as fan pages
              </h2>
              <p style={{ color: "#6b7280", fontSize: 12, margin: "4px 0 0" }}>
                Marque quais Facebook pages você quer conectar.
              </p>
            </div>
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          ...
                          {!p.instagram_username && (
                            <span style={{ color: "#9ca3af" }}> · sem Instagram</span>
                          )}
                        </p>
                      </div>
                    </button>
            ...
            >
              <p style={{ color: "#6b7280", fontSize: 12, margin: 0 }}>
                {pagesSelecionadas.size} de {pagesDisponiveis.length} selecionada(s)
              </p>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button
                  onClick={confirmarSelecaoPages}
                  disabled={conectandoMeta || pagesSelecionadas.size === 0}
                  style={{
                    background: conectandoMeta ? "#1d4ed8" : "#1877f2",
                    color: "white",
                    border: "none",
                    ...
                    fontWeight: 700,
                    cursor:
                      conectandoMeta || pagesSelecionadas.size === 0
                        ? "not-allowed"
                        : "pointer",
                    boxShadow: "0 4px 12px rgba(24,119,242,0.3)",
                  }}
                >
                  {conectandoMeta
                    ? "⏳ Conectando..."
                    : `📲 Conectar (${pagesSelecionadas.size})`}
                </button>
              </div>
            </div>
          </div>
